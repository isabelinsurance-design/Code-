...
case 'save_member':
    ...
    $all_fields = ['nombre','middle_name','apellido','telefono','telefono2','estado','subestado','agente_id',
        ...
        'dentista','email','evento','razon_cancelacion','fuente','referido_por',
    ...
    try {
        if (!empty($d['id'])) {
            ...
            $pdo->prepare("INSERT INTO actividad (agente_id,miembro_id,tipo,descripcion) VALUES (?,?,?,?)")
                ->execute([$uid,$d['id'],'SISTEMA','Perfil actualizado por '.$user['nombre']]);
            jsonOk(['id'=>$d['id'],'msg'=>'Miembro actualizado']);
        } else {
...
case 'save_reporte':
    $pdo = db();
    $d = $_POST;
    $pdo->prepare("INSERT INTO reporte_diario (agente_id,fecha,llamadas_prospectos,llamadas_servicio,citas_confirmadas,tickets_resueltos,apps_enviadas,polizas_escritas,nota,enviado)
        VALUES (?,?,?,?,?,?,?,?,?,1)
        ON DUPLICATE KEY UPDATE llamadas_prospectos=VALUES(llamadas_prospectos),llamadas_servicio=VALUES(llamadas_servicio),citas_confirmadas=VALUES(citas_confirmadas),tickets_resueltos=VALUES(tickets_resueltos),apps_enviadas=VALUES(apps_enviadas),polizas_escritas=VALUES(polizas_escritas),nota=VALUES(nota),enviado=1")
        ->execute([$uid,date('Y-m-d'),$d['llamadas_prospectos']??0,$d['llamadas_servicio']??0,$d['citas_confirmadas']??0,$d['tickets_resueltos']??0,$d['apps_enviadas']??0,$d['polizas_escritas']??0,$d['nota']??'']);
    jsonOk();
    break;
